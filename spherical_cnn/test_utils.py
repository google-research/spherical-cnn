...
import numpy as np
from spherical_cnn import np_spin_spherical_harmonics
...
@functools.lru_cache()
def _stacked_wigner_ds(ell_max: int,
                       alpha: float, beta: float, gamma: float) -> jnp.ndarray:
...
def rotate_coefficients(coefficients: Array,
                        alpha: float,
                        beta: float,
...
def get_spin_spherical(
    ...
    shape: Sequence[int],
    spins: Sequence[int]) -> Tuple[Array, Array]:
...
def apply_model_to_rotated_pairs(
    ...
    apply_args: Optional[Mapping[str, Any]] = None,
) -> Tuple[jnp.ndarray, jnp.ndarray, RotatedPair]:
