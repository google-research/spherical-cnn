...
def make_equiangular_grid(resolution: int) -> np.ndarray:
...
def torus_quadrature_weights(resolution: int) -> np.ndarray:
...
def spin_spherical_mean(sphere_set: Array) -> jnp.ndarray:
...
def ell_max_from_resolution(resolution: int) -> int:
...
@functools.lru_cache()
def compute_wigner_delta(ell: int) -> np.ndarray:
...
@functools.lru_cache()
def compute_all_wigner_delta(ell_max: int) -> Tuple[np.ndarray, ...]:
