...
class SpinSphericalBlock(nn.Module):
  ...
  Attributes:
    ...
    downsampling_factor: How much to downsample before applying the convolution.
      Will not downsample when downsampling_factor==1.
    spectral_pooling: When True, halve input dimensions via spectral pooling.
    ...
    after_conv_module: Module to apply after convolution. Usually a
      non-linearity or batch norm + non-linearity. Must follow the interface of
      `layers.SpinSphericalBatchNormMagnitudeNonlin`.
    transformer: SpinSphericalFourierTransformer instance.
    num_filter_params: Number of filter parameters in the convolutional layer.
    input_representation: Whether inputs are in the 'spectral' or 'spatial'
      domain.
    output_representation: Whether outputs fed into `after_conv_module` are in
      the 'spectral' or 'spatial' domain.
  """
  ...
  axis_name: Any
  transformer: spin_spherical_harmonics.SpinSphericalFourierTransformer
  after_conv_module: Type[nn.Module] = (
  ...
  def __call__(
    ...
    Args:
      ...
      weights: Weights per batch element used in batchnorm mean/std computation.
    ...
    if apply_spatial_pooling:
      ...
      feature_maps = SphericalPooling(
          stride=self.downsampling_factor, name="spherical_pool"
      )(feature_maps)

    feature_maps = SpinSphericalConvolution(
    ...
    return self.after_conv_module(
        ...
        axis_name=self.axis_name,
        transformer=self.transformer,
        name="batch_norm_nonlin",
    )(feature_maps, weights=weights)
...
class SpinSphericalResidualBlock(SpinSphericalBlock):
  ...
  def __call__(
    ...
    outputs = SpinSphericalSpectralBatchNormalization(
        **batch_norm_options,
    ...
    if residual.shape != self.num_channels:
      ...
      residual = SpinSphericalSpectralBatchNormalization(
